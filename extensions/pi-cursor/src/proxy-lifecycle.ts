...
// Use jiti (bundled with pi) to run the TypeScript proxy subprocess.
// The proxy is a standalone HTTP server that must run as its own process
// because it holds persistent H2 connections and conversation state.
const JITI_CLI = join(homedir(), '.pi', 'agent', 'npm', 'node_modules', 'jiti', 'lib', 'jiti-cli.mjs')
...
async function spawnProxy(sessionId: string, accessToken: string): Promise<{ port: number; models: CursorModel[] }> {
  const child = spawn(process.execPath, [JITI_CLI, PROXY_ENTRY], {
