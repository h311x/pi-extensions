...
export function resolveModelId(
  ...
  if (effort) {
    ...
    // Note: Do NOT append effort to models that lack effort variants.
    // Models like `composer-2.5-fast` have no effort maps — injecting
    // `-high` would produce a nonexistent model ID that Cursor rejects
    // with `not_found`.
  }
